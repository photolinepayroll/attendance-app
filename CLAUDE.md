...
- **Clock integrity**: A manually-set phone clock would otherwise poison the displayed time, the selfie's burned-in watermark, and the record's `ts`. `checkNetworkTime()` fetches the page's own URL (same-origin `HEAD`, no backend involved) and reads the HTTP `Date` response header as a trusted time source, storing the drift in `clockOffsetMs`. `trustedNow()` (used everywhere a record/display timestamp is generated, in place of `new Date()`) applies that offset so the clock self-corrects even before the phone setting is fixed. `isClockVerified()` requires a successful check within the last ~35s — an unreachable/offline phone counts as unverified, not "fine", so Log In/Out stays blocked (via `isClockGateBlocked()`, checked in `checkInputs()`, `selectAction()`, `openCamera()`, and again in `submitAttendance()`) until it can actually confirm the clock, closing the "turn off data to dodge verification" loophole. On top of that, `hasClockJumped()` compares elapsed `performance.now()` (monotonic, immune to the user changing the system clock) against elapsed `Date.now()` since the last verified anchor — any divergence means the wall clock was manually changed *after* a successful check (e.g. verify while online, then go offline and tamper), and folds into `isClockBad()` so it's caught within ~1s via the existing 1Hz `updateClock()` tick, without needing a network round-trip.
